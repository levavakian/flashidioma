...
- **Starts as "learning":** auto-added cards start in the FSRS "learning" state (not "new"), so they appear in the review queue immediately without consuming a daily new-card slot
...
- [ ] When the review queue is exhausted, check for learning/relearning cards that have become due during the session
- [ ] If any exist, add them to the queue and continue the session
- [ ] Show a brief waiting message if cards are due soon but not yet (e.g. "Next card due in 1m")
- [ ] Session only completes when no more cards are due and none are pending
- **Tests:**
  - [ ] Unit: grading "Again" on a card puts it in learning/relearning state with short due interval
  - [ ] Component: grading "Again" on all cards re-shows them in the same session (session doesn't end prematurely)

#### 14b: Auto-Added Conjugation Cards Start as "Learning" (Not "New")
Cards generated by the auto-conjugation feature (Good/Easy on a verb) should start in a review-ready state, not "new":
- [ ] Add a new card source type `'auto-conjugation'` to distinguish from manual/imported/practice
- [ ] Auto-added conjugation cards start with FSRS state = `'learning'` and `dueDate = now` (immediately available for review)
- [ ] Auto-added cards do NOT count against the daily new card limit (`newCardsPerDay`)
- [ ] Auto-added cards do NOT consume a batch slot (`currentBatchCardIds`)
- **Tests:**
  - [ ] Unit: auto-added conjugation card has state 'learning', not 'new'
  - [ ] Unit: auto-added conjugation card does not increment daily new card count
  - [ ] Unit: auto-added card appears in `getDueCards` (not filtered by new-card gate)
...
| # | Description | Status |
|---|-------------|--------|
| 1 | "Again" grading doesn't re-queue cards in same session — session ends prematurely | Fix planned (14a) |
