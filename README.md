...
- **Daily new-card limit:** manual, imported, practice, and new auto-conjugation cards all occupy the same `newCardsPerDay` daily set when they are introduced into the review queue. Cards do not wait until grading to consume the daily allowance.
- **Daily-set cleanup and legacy repair:** when a daily new-card set is finished, stale `currentBatchCardIds` are cleared even if the daily limit prevents another set from being introduced yet. Deck Settings includes an "Update Deck Schema" button that inserts any missing current defaults for older persisted decks, clears any polluted active new-card queue, recomputes the active daily set from cards due inside the review-day boundary and `newCardsPerDay`, updates the daily counter to match, and reports whether anything changed.
- **Auto-conjugation cards:** by default, auto-conjugation cards start as `new` and flow through the next daily new-card set, occupying `newCardsPerDay` slots like other new cards. If `conjugationCardsStartLearning` is enabled, they start as `learning`, are immediately reviewable, and enter the due queue without consuming a new-card slot.
...
- **Daily limit:** configurable max conjugation cards added per day (default: 5) to avoid generating too many new cards at once. If those generated cards start as `new`, they are still introduced later through the deck's `newCardsPerDay` review limit.
...
- [x] Auto-added cards that start as `learning` do NOT consume `newCardsPerDay` slots
- [x] Auto-added cards that start as `new` are introduced through the same `newCardsPerDay` daily set as other new cards
- **Tests:**
  ...
  - [x] Unit: auto-added conjugation card does not increment daily new card count
  - [x] Unit: with `conjugationCardsStartLearning`, auto-added card appears in `getDueCards` (not filtered by new-card gate)

#### 14c: Show Due Date in Card List View
For cards that are not in 'new' state, show when they are next due in the deck's card list view.
- [ ] Display relative due date (e.g. "Due in 2d", "Due now", "Overdue 3d") next to the state badge
- [ ] Cards that are due now or overdue should be visually highlighted
- **Tests:**
  - [ ] Component: card list shows due date for review/learning/relearning cards but not for new cards

#### 14d: "Skip Forward One Day" Button in Deck Settings
Add a button that simulates a day passing, making tomorrow's due cards available for review today.
- [ ] Add "Skip Forward One Day" button in the deck settings page
- [ ] On click, subtract 24 hours from the `dueDate` of every card in the deck
- [ ] Also subtract 24 hours from `lastReview` for consistency
- [ ] Also reset the daily counters (`newCardsIntroducedToday`, `conjugationCardsAddedToday`, and `lastNewCardDate`/`lastConjugationCardDate`) as if a new day started
...
| # | Description | Status |
|---|-------------|--------|
| 1 | "Again" grading doesn't re-queue cards in same session — session ends prematurely | Fixed |
| 2 | Mobile deck creation may fail on Android Chrome/PWA due to DB version upgrade blocking | Mitigation added (error display + force reset) |
| 3 | Practice "Add as Card" saved target-to-source sentences as source-to-target cards | Fixed |
...
| 11 | Auto-added conjugation cards nested the previous card's bracket annotation when translation was unavailable, e.g. `you commented [to comment (tú preterite)] [to you commented [to comment (tú preterite)] (ellos/ellas/ustedes present subjunctive)]` | Fixed (DB v4 upgrade repairs existing labels) |
