...
- **Daily-set cleanup and legacy repair:** when a daily new-card set is finished, stale `currentBatchCardIds` are cleared even if the daily limit prevents another set from being introduced yet. If older state polluted `currentBatchCardIds` with too many new cards, the active set is trimmed back to cards due inside the review-day boundary and the deck's daily allowance. Deck Settings includes an "Update Deck Schema" button that inserts any missing current defaults for older persisted decks, trims polluted active sets, and reports whether anything changed.
